...
实时模式当前本质上是“数据同步驱动的 slot 对齐执行”。

流程是：

1. 创建 `live_task`
2. API 进程内 `market sync loop` 或外部 `market-sync worker` 定期向交易所做增量同步
3. 只有当一轮 sync 全量成功且本地 `coverage_end_ms` 前进时，才进入 live dispatch
4. dispatcher 按 task cadence 把最新 `coverage_end_ms` 对齐到可执行 slot
5. API 用该 slot 的 `as_of_ms` 构造严格市场快照并调用 Runner
6. Runner 返回决策
7. Portfolio Engine 更新实时 scope 下的组合
8. 保存一条 `LiveSignal`

这里要注意：

- 它当前不是交易所实时流式执行
- 它也不再是 wall-clock scheduler 直接触发
- 更准确地说是“基于已落库真实行情的 sync-driven 准实时执行”
...
当前仓库仍然偏向本地单机 demo，但底层 runtime 已经切到 PostgreSQL：

- PostgreSQL 作为唯一运行时数据库
- 回测与实时执行状态通过数据库 claim / revision 协调
- API 进程内 `market sync loop` 仍可运行，但也支持外部 `market-sync worker`
- Runner 独立但通常仍然作为本地服务调试

这使得开发、调试和演示成本仍然较低，同时为更高并发和更大数据量打下了正确的数据平面基础。
...
- 回测执行的 durable claim / checkpoint 已经是 PostgreSQL-backed，但默认仍由 API 侧触发，不是完整独立 job fleet
...
- `docs/postgresql-runtime-operations.md`
