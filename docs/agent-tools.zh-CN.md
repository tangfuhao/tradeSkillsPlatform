...
| `save_strategy_state` | 策略状态 | Runner 本地暂存 + API 成功提交 | 暂存状态 patch，供本轮成功结束后统一落库 |
...
- Runner 只会更新本地 `pending_state_patch`，不会在工具循环里直接写库
- 返回的 `strategy_state` 是“已持久化状态 + 本轮 staged patch”的合并预览
- 只有当本轮 trigger 成功结束后，API 才会把最终 `decision.state_patch` 提交到 execution scope
...
1. 通过 `PortfolioEngine` 执行模拟开平仓/减仓/盯市
2. 在回测模式下写入 trace，或在实时模式下写入 live signal
3. 把最终 `state_patch`、组合变化和 trace/signal 作为同一批结果统一提交
...
5. 需要时用 `save_strategy_state` 暂存状态 patch
